...
function TimeSlotViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TimeSlotInterface>(
    ...
    () =>
      getTimeSlotById(id, {
        relations: ['business', 'appointment'],
      }),
  );
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('time_slot', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/time-slots/edit/${data?.id}`} passHref legacyBehavior>
              <Button
              ...
              {hasAccess('appointment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/appointments/create?time_slot_id=${data?.id}`}>
