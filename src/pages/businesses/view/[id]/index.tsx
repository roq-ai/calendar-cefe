...
import { deleteTimeSlotById } from 'apiSdk/time-slots';
...
function BusinessViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface>(
    ...
    () =>
      getBusinessById(id, {
        relations: ['user', 'time_slot'],
      }),
  );

  const time_slotHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('time_slot', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/time-slots/create?business_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.time_slot?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/time-slots/view/${record.id}`)}
                            key={record.id}
                          >
                            ...
                            <Td>
                              {hasAccess('time_slot', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/time-slots/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('time_slot', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    time_slotHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
